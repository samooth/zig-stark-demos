//! WASM C-ABI for the Binius sorted-sequence demo.
//!
//! Exposes prove/verify over the canonical serialized wire format
//! (see zig-stark docs/wire.md). The constraint system (two 4-bit range
//! checks + one 4-bit comparison, linked) is baked in at comptime; only
//! the witness columns and boundary pins vary per invocation.
//!
...
pub export fn zs_version() callconv(.c) [*:0]const u8 {
    return "binius-sorted-seq 0.1.0 (Gf256/Gf2_128)";
}
